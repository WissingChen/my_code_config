...
文档冲突时的优先级：`AGENTS.md`（下游项目根，你手写的目标、工具链、约束与过程政策，Agent 只读；Agent 最多只能把仓库根 `AGENTS_template.md` 复制为项目根 `AGENTS.md`，内容全由你填写）→ `global.md`（纯索引与指针，不含目标和规则）→ 方向 `00-overview.md`（当前状态，含主矛盾与停放区）→ 已登记实验计划（冻结预期）→ 运行报告（观测证据）→ 文献笔记（外部证据）→ 生成的状态快照（一次性视图）。
