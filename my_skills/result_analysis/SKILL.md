...
候选图和选中/拒绝结论写进该报告唯一的一份配图规划（`write_md` §3 第一遍维护），本节不另起一份记录。选中的图交给 `result_visualization` 时带齐：用途、数据来源/版本、观测单位和设计、变量/分组/分面/排序、汇总/变换方式、合法的不确定度表示、主张或对比、目标媒介和渲染器、图的类别和保留建议。交接中不许改动统计结论。
