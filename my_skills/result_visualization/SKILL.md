...
| 随步数趋势 | 折线 + 标明不确定度单位 |
| 比较 | 点图、小倍数图；类别少才用柱状 |
| 案例 × 条件矩阵 | 带数值标注的热图；差值用 0 居中的双色 |
| 报告级总览 | 多面板合成图：一个主张一张图，面板共享轴与图例 |
| 关系 | 散点、热图、等高线 |
| 校准 | 可靠性图、残差图 |
| 敏感性/消融 | 有序点/柱状、交互图 |
| baseline–预期–实际 | 三个都标出的区间对比图 |

**折线只用于有序自变量的趋势**（步数、轮次、时间）；横截面组间比较用点图、柱状、热图或斜率图，不把无序组别摆上折线的 x 轴。

**现象优先**：动笔前先用一句话写出这张图要凸显的现象，编码必须让它第一眼可见——按效应量排序、画参考线（基线/canon/天花板/地板）、关键差距和离群点直接标注在图上、对比组用最饱和的颜色而其余压灰、分布（散点）与聚合（均值条）叠加。报告级证据图默认高密度：热图 cell 带数值、多面板合成（一个主张一张图）；单条裸折线只配当诊断图，不进报告。
...
产出可复现源文件加最少输出：矢量用 PDF/SVG，确实需要预览或位图时才出 PNG。逐项检查：轴标签和单位、刻度和基线、图例、排序、不确定度含义、色盲/灰度可读性、字体可用性、裁切、图注自足性——清单见 `plotting-reference.md` §10。**嵌入处的渲染验收按 `experiment_manager` §4a 的标准执行——磁盘上一个正确但没人引用的文件不算交付。**
